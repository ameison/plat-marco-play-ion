starter.controller('MonitoreoTemperaturaCtrl', function($scope, $rootScope, $http, myConfig, localStorageService, $ionicModal,
  ...
  $ionicPlatform.ready(function () {
    ...
    $scope.obtenerModelosXCategoria = function(categoriaId, superintendencia_id){
      ...
      console.log("SUPERINTENDENCIA : " + superintendencia_id);
    ...
    $scope.obtenerFormatosXModeloYMina = function(){
      ...
      $cordovaSQLite.execute(db, queryFur, [$scope.data.modelo, $scope.data.mina, myConfig.tipoInspeccion[1].id]).then(function(res) {
    ...
    $scope.nuevaInspeccion = function(){
      ...
      if ($scope.data.mina != undefined && $scope.data.superintendencia != undefined
        ...
        var nuevo = {
          ...
          "tipo": myConfig.tipoInspeccion[1].id,
          ...
          "orden_trabajo": '',
          "horometro": 0,
          "resumen_actividad": 'Monitoreo de temperatura',
          "aprobado_soporte": 0,
          "compartido": 0,
          "sincronizado": 0,
    ...
    $scope.listarInspeccionesMT = function(){
      try {
        ...
        $cordovaSQLite.execute(db, queryFur, [myConfig.tipoInspeccion[1].id, myConfig.estadoInspeccion[0].id, myConfig.estadoInspeccion[1].id]).then(function(res) {
          $scope.inspecciones = [];
          for (var i = 0; i < res.rows.length; i++) {
            ...
            console.log("MONITOREO TEMPERATURA", JSON.stringify($scope.inspecciones));
          }
        }, function (err) {
          console.error(JSON.stringify(err));
        });
    ...
    $scope.listarInspeccionesMTHistorico = function(){
      try {
        ...
        $cordovaSQLite.execute(db, queryFur, [myConfig.tipoInspeccion[1].id, myConfig.estadoInspeccion[2].id]).then(function(res) {
    ...
    $scope.descargarInspeccionesMT = function(){
      if (conexion) {
              ...
              $cordovaSQLite.execute(db, queryDel, []).then(function(res) {
                ...
                for (var i = 0; i < data.length; i++) {
                  ...
                  $cordovaSQLite.execute(db, query, [nuevo.estado, nuevo.tipo, nuevo.fur_id, nuevo.superintendencia_id, nuevo.modelo_id,
                    ...
                    $scope.listarInspeccionesMT();
                    $scope.listarInspeccionesMTHistorico();
                  }, function (err) {
                    console.error(JSON.stringify(err));
                  });
                }
    ...
    $rootScope.$on('$stateChangeSuccess', function(event, toState, toParams, fromState, fromParams) {
      if (toState.name == 'app.monitoreo-temperatura') {
        console.log("ENTRO POR app.monitoreo-temperatura");
        $scope.listarInspeccionesMT();
        $scope.listarInspeccionesMTHistorico();
      }
    });
