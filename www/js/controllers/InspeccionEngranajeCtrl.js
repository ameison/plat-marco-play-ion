starter.controller('InspeccionEngranajeCtrl', function($scope, $rootScope, $http, myConfig, localStorageService, $ionicModal,
  $cordovaSQLite, $location, $state, InspeccionFactory, $cordovaToast, $cordovaNetwork, $ionicPlatform) {
  ...
  $ionicPlatform.ready(function () {
    ...
    $scope.obtenerFormatosXModeloYMina = function(){
      ...
      $cordovaSQLite.execute(db, queryFur, [$scope.data.modelo, $scope.data.mina, myConfig.tipoInspeccion[4].id]).then(function(res) {
    ...
    $scope.nuevaInspeccion = function(){
      ...
      if ($scope.data.mina != undefined && $scope.data.superintendencia != undefined
        ...
        var nuevo = {
          ...
          "tipo": myConfig.tipoInspeccion[3].id,
          ...
          "orden_trabajo": '',
          "horometro": 0,
          "resumen_actividad": 'Inspección engranaje',
          "aprobado_soporte": 0,
          "compartido": 0,
          "sincronizado": 0,
    ...
    $scope.listarInspeccionesEN = function(){
      try {
        console.log("Listado de inspecciones pendientes y observadas");
        var queryFur = "SELECT ins.id, ins.estado, ins.tipo, ins.superintendencia_id, ins.equipo_id, ins.servidor_id, ins.compartido, ins.sincronizado, ins.resumen_actividad, ins.responsable_id, fecha_creacion, ins.responsable_nombres, spi.nombre as sup_nombre, mis.nombre as min_nombre, eq.nombre as eq_nombre FROM inspecciones as ins LEFT JOIN superintendencias as spi ON ins.superintendencia_id = spi.id LEFT JOIN minas as mis ON spi.mina_id = mis.id LEFT JOIN equipos as eq ON eq.id = ins.equipo_id WHERE ins.tipo = ? and ins.estado in (?,?)";
        $cordovaSQLite.execute(db, queryFur, [myConfig.tipoInspeccion[3].id, myConfig.estadoInspeccion[0].id, myConfig.estadoInspeccion[1].id]).then(function(res) {
          $scope.inspecciones = [];
          for (var i = 0; i < res.rows.length; i++) {
    ...
    $scope.listarInspeccionesENHistorico = function(){
      try {
        ...
        $cordovaSQLite.execute(db, queryFur, [myConfig.tipoInspeccion[3].id, myConfig.estadoInspeccion[2].id]).then(function(res) {
    ...
    $scope.descargarInspeccionesEN = function(){
      if (conexion) {
                ...
                for (var i = 0; i < data.length; i++) {
                  ...
                  $cordovaSQLite.execute(db, query, [nuevo.estado, nuevo.tipo, nuevo.fur_id, nuevo.superintendencia_id, nuevo.modelo_id,
                    ...
                    nuevo.responsable_nombres, nuevo.equipo_id, nuevo.fecha_creacion, nuevo.observaciones]).then(function(res) {
                      $scope.listarInspeccionesEN();
                      $scope.listarInspeccionesENHistorico();
                  }, function (err) {
                    console.error(JSON.stringify(err));
                  });
                }
    ...
    $rootScope.$on('$stateChangeSuccess', function(event, toState, toParams, fromState, fromParams) {
      if (toState.name == 'app.inspeccion-engranaje') {
