starter.controller('MantenimientoCorrectivoCtrl', function($scope, $rootScope, $http, myConfig, localStorageService, $ionicModal,
  ...
  $ionicPlatform.ready(function () {
    ...
    $scope.obtenerFormatosXModeloYMina = function(){
      ...
      $cordovaSQLite.execute(db, queryFur, [$scope.data.modelo, $scope.data.mina, myConfig.tipoInspeccion[2].id]).then(function(res) {
    ...
    $scope.nuevaInspeccion = function(){
      ...
      if ($scope.data.mina != undefined && $scope.data.superintendencia != undefined
        ...
        var nuevo = {
          ...
          "tipo": myConfig.tipoInspeccion[2].id,
        ...
        $cordovaSQLite.execute(db, queryFur, [myConfig.tipoInspeccion[2].id, myConfig.estadoInspeccion[0].id, myConfig.estadoInspeccion[1].id]).then(function(res) {
          ...
          for (var i = 0; i < res.rows.length; i++) {
            $scope.inspecciones.push({
              ...
              "inspeccion_cerrada_id": res.rows.item(i).inspeccion_cerrada_id,
              "resumen_actividad": res.rows.item(i).resumen_actividad
            });
            console.log("INSPECCIONES CORRECTIVA", JSON.stringify($scope.inspecciones));
          }
        }, function (err) {
          console.error(JSON.stringify(err));
        });
    ...
    $scope.listarInspeccionesCOHistorico = function(){
      try {
        ...
        $cordovaSQLite.execute(db, queryFur, [myConfig.tipoInspeccion[2].id, myConfig.estadoInspeccion[2].id]).then(function(res) {
    ...
    $scope.descargarInspeccionesCO = function(){
      if (conexion) {
            ...
            InspeccionFactory.descargarSincInspecciones(function(data){
              ...
              $cordovaSQLite.execute(db, queryDel, []).then(function(res) {
                for (var i = 0; i < data.length; i++) {
                  ...
                  if (inspeccion.inspeccionCerrada != null && inspeccion.inspeccionCerrada != undefined){
                    console.log("El mantenimiento correctivo esta cerrando otra inspección");
                    nuevo.inspeccion_cerrada_id = inspeccion.inspeccionCerrada.id;
                  }else{
                    console.log("El mantenimiento correctivo no se ha cerrado");
                  }

                  var query = "INSERT INTO inspecciones (estado, tipo, fur_id, superintendencia_id, modelo_id, contenido, prioridad, servidor_id, orden_trabajo, horometro, resumen_actividad, aprobado_soporte, compartido, sincronizado, responsable_id, responsable_nombres, equipo_id, fecha_creacion, inspeccion_cerrada_id, observaciones) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
                  $cordovaSQLite.execute(db, query, [nuevo.estado, nuevo.tipo, nuevo.fur_id, nuevo.superintendencia_id, nuevo.modelo_id,
                    JSON.stringify(nuevo.contenido), nuevo.prioridad, nuevo.servidor_id, nuevo.orden_trabajo, nuevo.horometro,
                    nuevo.resumen_actividad, nuevo.aprobado_soporte, nuevo.compartido, nuevo.sincronizado, nuevo.responsable_id,
                    nuevo.responsable_nombres, nuevo.equipo_id, nuevo.fecha_creacion, nuevo.inspeccion_cerrada_id, nuevo.observaciones]).then(function(res) {

                    $scope.listarInspeccionesCO();
                    $scope.listarInspeccionesCOHistorico();
    ...
    $scope.sincronizarInspeccion = function(inspeccionId){
      ...
      var query = "SELECT id, equipo_id, tipo, superintendencia_id, modelo_id, contenido, prioridad, resumen_actividad, orden_trabajo, horometro, fur_id, compartido, responsable_id, servidor_id, inspeccion_cerrada_id FROM inspecciones where id = ?";
      $cordovaSQLite.execute(db, query, [inspeccionId]).then(function(res) {
        ...
        if (inspeccion != undefined){
          console.log("responsable_id", inspeccion.responsable_id);
          var data = {
            "tipo": myConfig.tipoInspeccion[2].id,
            ...
            "inspeccionCerrada": {
              "id": inspeccion.inspeccion_cerrada_id
            }
          }
    ...
    $scope.actualizarInspeccionNuevo = function(inspeccionId, response){
      ...
      if (response.hasOwnProperty("inspeccion_cerrada_id")){
        console.log("Se ha cerrado la inspeccion id : ", response.inspeccion_cerrada_id);
        var queryUpd = "UPDATE inspecciones set estado = ? WHERE servidor_id in (?, ?)";
        $cordovaSQLite.execute(db, queryUpd, [myConfig.estadoInspeccion[2].id, response.servidor_id, response.inspeccion_cerrada_id]).then(function(res) {
          console.log("FORMATO ACTUALIZADO : " + JSON.stringify(res));
          $scope.listarInspeccionesCO();
          $scope.listarInspeccionesCOHistorico();
        }, function (err) {
        ...
        console.log("Esta inspección no cierra nada");
      }
    }
    ...
    $rootScope.$on('$stateChangeSuccess', function(event, toState, toParams, fromState, fromParams) {
      if (toState.name == 'app.mantenimiento-correctivo') {
        console.log("ENTRO POR app.mantenimiento-correctivo");
        $scope.listarInspeccionesCO();
        $scope.listarInspeccionesCOHistorico();
      }
    });

    $scope.cerrarInspeccionCorrectiva = function(inspeccionId, servidor_id){
      console.log("Cerrar inspeccion con id ", inspeccionId);
      console.log("Cerrar inspeccion con servidor id ", servidor_id);
      var queryFind = "SELECT id FROM inspecciones where inspeccion_cerrada_id = ?";
      $cordovaSQLite.execute(db, queryFind, [servidor_id]).then(function(res) {
        console.log("CIERRE CORRECTIVO", JSON.stringify(res));
        if (res.rows.length > 0){
          $scope.showToast("Ya existe una inspección que cierra esta inspección.");
        }else{
          $cordovaDialogs.confirm('¿Seguro que deseas cerrar esta inspección?', 'MENSAJE', ['Cancelar','Aceptar']).then(function(index){
            switch (index) {
              case 2:
                var query = "SELECT equipo_id, superintendencia_id, modelo_id, fur_id, servidor_id FROM inspecciones where id = ?";
                $cordovaSQLite.execute(db, query, [inspeccionId]).then(function(res) {
                  var inspeccion = undefined;
                  for (var i = 0; i < res.rows.length; i++) {
                    inspeccion = {
                      "equipo": res.rows.item(i).equipo_id,
                  ...
                  if (inspeccion != undefined && inspeccion.servidor_id != undefined){
                    var queryFur = "SELECT contenido FROM furs where id = ?";
                    $cordovaSQLite.execute(db, queryFur, [inspeccion.fur]).then(function(resFur) {
                      var contenido = undefined;
                      for (var i = 0; i < resFur.rows.length; i++) {
                        contenido = resFur.rows.item(i).contenido;
                        break;
                      }
                      if (contenido != undefined){
                        var nuevo = {
                          "estado": myConfig.estadoInspeccion[0].id,
                          "tipo": myConfig.tipoInspeccion[2].id,
                          "fur_id": inspeccion.fur,
                          "superintendencia_id": inspeccion.superintendencia,
                          "modelo_id": inspeccion.modelo,
                          "contenido": JSON.parse(contenido),
                          "prioridad": myConfig.prioridadInspeccion[1].id,
                          "servidor_id": 0,
                          "orden_trabajo": '',
                          ...
                          "responsable_nombres": localStorageService.get("nombres"),
                          "inspeccion_cerrada_id": 0,
                          "equipo_id": inspeccion.equipo
                        }

                        var query = "INSERT INTO inspecciones (estado, tipo, fur_id, superintendencia_id, modelo_id, contenido, prioridad, servidor_id, orden_trabajo, horometro, resumen_actividad, aprobado_soporte, compartido, sincronizado, responsable_id, responsable_nombres, inspeccion_cerrada_id, equipo_id) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
                        $cordovaSQLite.execute(db, query, [nuevo.estado, nuevo.tipo, nuevo.fur_id, nuevo.superintendencia_id, nuevo.modelo_id,
                          JSON.stringify(nuevo.contenido), nuevo.prioridad, nuevo.servidor_id, nuevo.orden_trabajo, nuevo.horometro,
                          nuevo.resumen_actividad, nuevo.aprobado_soporte, nuevo.compartido, nuevo.sincronizado, nuevo.responsable_id,
                          nuevo.responsable_nombres, inspeccion.servidor_id, nuevo.equipo_id]).then(function(resIns) {

                          $scope.listarInspeccionesCO();
                          $scope.showToast("Inspección de cierre creado exitosamente.");
                        ...
                        console.log("La inspeccion o el contenido del fur son nulos");
                      }
                    ...
                    $scope.showToast("Para cerrar una inspeccion debes obtener el CODIGO DE SERVIDOR.");
                  }
        ...
        $scope.showToast("Ocurrio un error al cerrar inspección, comuniquese con el proveedor.");
        console.error(JSON.stringify(err));
      });
